...
export function ImagesManagerModal({ feedbackItemId, initialImages }: Props) {
  ...
  const refreshImages = useCallback(async () => {
    try {
      const resp = await fetch(`/api/images/list-by-parent?scope=feedback_item&parentId=${feedbackItemId}`)
      if (resp.ok) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('open-images-modal', handler)
    return () => window.removeEventListener('open-images-modal', handler)
  }, [refreshImages])
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogContent className="sm:max-w-2xl bg-white">
        <DialogHeader>
          <DialogTitle>Explanatory Images</DialogTitle>
        </DialogHeader>
